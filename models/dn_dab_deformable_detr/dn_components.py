...
def prepare_for_dn(dn_args, tgt_weight, embedweight, batch_size, training, num_queries, num_classes, hidden_dim, label_enc):
    ...
    # debug
    # tgt_weight = tgt_weight.cuda()
    # indicator0 = indicator0.cuda()
    # label_enc.weight = label_enc.weight.cuda()
    ...
    tgt = torch.cat([tgt_weight, indicator0], dim=1) + label_enc.weight[0][0].cpu()*torch.tensor(0).cpu()
    ...
    return input_query_label, input_query_bbox, attn_mask, mask_dict  # mask
...
def tgt_loss_boxes(src_boxes, tgt_boxes, num_tgt,):
    ...
    losses = {}
    losses['tgt_loss_bbox'] = loss_bbox.sum() / num_tgt #L1 loss

    loss_giou = 1 - torch.diag(box_ops.generalized_box_iou(
        box_ops.box_cxcywh_to_xyxy(src_boxes),
        box_ops.box_cxcywh_to_xyxy(tgt_boxes)))
    losses['tgt_loss_giou'] = loss_giou.sum() / num_tgt #GIoU loss
    # assert 1==0
    return losses
...
def tgt_loss_labels(src_logits_, tgt_labels_, num_tgt, focal_alpha, log=True):
    ...
    loss_ce = sigmoid_focal_loss(src_logits, target_classes_onehot, num_tgt, alpha=focal_alpha, gamma=2) * src_logits.shape[1]  #sigmoid_focal_loss
    # assert 1==0
...
def compute_dn_loss(mask_dict, training, aux_num, focal_alpha):
    ...
    if training and 'output_known_lbs_bboxes' in mask_dict:
        ...
        num_tgt = prepare_for_loss(mask_dict)  # prepare_for_loss(mask_dict)   known_labels, known_bboxs, output_known_class, output_known_coord
