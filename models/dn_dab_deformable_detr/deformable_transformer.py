...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        self.decoder = DeformableTransformerDecoder(decoder_layer, num_decoder_layers, return_intermediate_dec, 
                                                        use_dab=use_dab, d_model=d_model, high_dim_query_update=high_dim_query_update, no_sine_embed=no_sine_embed,inquiry_head_num=4, mi_mechanism=True) # inquiry_head_num=4, mi_mechanism=True
        ...
        # dense connection concat
        self.u_fusions = nn.Sequential(
            nn.Linear(2 * d_model, d_model),
            nn.LayerNorm(d_model),
        )
        self.u_fusions = nn.ModuleList([copy.deepcopy(self.u_fusions) for i in range(num_encoder_layers - 1)])
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, attn_mask=None):
        ...
        memory, query_per_layer = self.encoder(src_flatten, spatial_shapes, level_start_index, valid_ratios, lvl_pos_embed_flatten, mask_flatten)
        

        # Dense Feature Interaction 
        last = query_per_layer[-1]  # last
        last2 = query_per_layer[-2]
        last3 = query_per_layer[-3]
        last4 = query_per_layer[-4]
        last5 = query_per_layer[-5]
        last6 = query_per_layer[-6]


        fused = []
        for i in range(len(query_per_layer) - 1):
            cat_i = torch.cat([last, query_per_layer[i]], dim=-1)  #         # (B, L, 2C)
            fused_i = self.u_fusions[i](cat_i)                                 # (B, L, C)
            
            cat_i = torch.cat([last2, fused_i], dim=-1)  # (B, L, 2C)
            fused_i = self.u_fusions[i](cat_i)                     # (B, L, C)
            
            cat_i = torch.cat([last3, fused_i], dim=-1)  # (B, L, 2C)
            fused_i = self.u_fusions[i](cat_i)                     # (B, L, C)
            
            cat_i = torch.cat([last4, fused_i], dim=-1)  # (B, L, 2C)
            fused_i = self.u_fusions[i](cat_i)                     # (B, L, C)
            
            cat_i = torch.cat([last5, fused_i], dim=-1)  # (B, L, 2C)
            fused_i = self.u_fusions[i](cat_i)                     # (B, L, C)
            
            cat_i = torch.cat([last6, fused_i], dim=-1)  # (B, L, 2C)
            fused_i = self.u_fusions[i](cat_i)                     # (B, L, C)
                
            fused.append(fused_i)
        fused.append(last)
        memory_per_layer = fused[::-1]  # len == num_encoder_layers
        ...
        hs, inter_references = self.decoder(tgt, reference_points, memory,
                                            ...
                                            src_padding_mask=mask_flatten, attn_mask=attn_mask,query_per_layer=memory_per_layer)  #query_per_layer=query_per_layer 新加的   # <--- 关键：把融合后的每层送给 decoder
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None):
        ...
        for _, layer in enumerate(self.layers):
            output = layer(output, pos, reference_points, spatial_shapes, level_start_index, padding_mask)
            query_per_layer.append(output)
        return output,query_per_layer
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False, use_dab=False, d_model=256, high_dim_query_update=False, no_sine_embed=False,
                inquiry_head_num=4, mi_mechanism=True):   
        super().__init__()
        ...
        self.mi_mechanism = True
        self.inquiry_head_num = inquiry_head_num
        self.query_per_layer = None
        # query fusion
        self.state_aware_query_fusion = nn.Sequential(
            nn.Linear(inquiry_head_num * d_model, d_model),
            nn.LayerNorm(d_model),
        )
        self.inquiry_head_num = inquiry_head_num      
        # inquiry
        for i in range(2, self.inquiry_head_num + 1):
            setattr(self, f"layers_for_branch{i}", _get_clones(decoder_layer, num_layers))  
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes,       
                ...
                query_pos=None, src_padding_mask=None, attn_mask=None,query_per_layer=None):
        # bs, num_queries, _ = tgt.size() 
        output = tgt 
        if self.use_dab:
        ...
        # if self.mi_mechanism:
        #     mi_layers = [self.layers]
        #     for i in range(2, self.inquiry_head_num + 1):
        #         mi_layers.append(getattr(self, f"layers_for_branch{i}"))
        #     mi_layers = zip(*mi_layers)
        # else:
        #     mi_layers = [(layer,) for layer in self.layers]
        ...
        for lid, inquiry_heads in enumerate(mi_layers):
            # import ipdb; ipdb.set_trace()
            ...
            for inquiry_head in inquiry_heads:
                mem = query_per_layer[lid] if query_per_layer is not None else src
                inquiry_out = inquiry_head(
                    output, query_pos, reference_points_input,
                    mem, src_spatial_shapes, src_level_start_index,
                    src_padding_mask, self_attn_mask=attn_mask
                )
                branch_outputs.append(inquiry_out)
            # fusion
            if self.mi_mechanism:
                output = self.state_aware_query_fusion(torch.cat(branch_outputs, dim=-1))
            else:
