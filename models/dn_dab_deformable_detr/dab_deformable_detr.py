...
from .dbffm import DualBranchFusionBlock
...
class DABDeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        self.fftls = DualBranchFusionBlock(dim=hidden_dim,alpha=0.7)  #DBFFM
    ...
    def forward(self, samples: NestedTensor, dn_args=None):
        ...
        srcs[1] = srcs[1] + F.interpolate(self.fftls(srcs[0]), size=(srcs[1].shape[2], srcs[1].shape[3]))  #[2,256,38,51] [2,256,38,51] [2,256,76,102]{srcs[0]最底层特征，最细粒度}
        srcs[2] = srcs[2] + F.interpolate(self.fftls(srcs[1]), size=(srcs[2].shape[2], srcs[2].shape[3]))  #[2,256,19,26] [2,256,19,26] [2,256,38,51]{srcs[1]}
        srcs[3] = srcs[3] + F.interpolate(self.fftls(srcs[2]), size=(srcs[3].shape[2], srcs[3].shape[3]))  #[2,256,10,13] [2,256,10,13]{srcs[3]} [2,256,19,26]{srcs[2]}
